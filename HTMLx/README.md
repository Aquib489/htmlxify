# HTMLx - Web Markup Compiler

A complete, production-ready compiler for **HTMLx** - a simplified web markup language that transpiles to clean HTML, CSS, and JavaScript with no configuration needed.

> **Write cleaner, faster markup. Build modern web pages in seconds.**

## 🌟 What is HTMLx?

HTMLx is a lightweight markup language designed to make web development faster and cleaner. Instead of writing verbose HTML, you write simple, intuitive markup that compiles to production-ready HTML, CSS, and JavaScript.
...
Write this in a file named `page.htmlx`:
...
htmlx hello.htmlx output/ --verbose  # With verbose output
htmlx --version                   # Check version
htmlx --help                      # View help
```
...
python cli.py hello.htmlx output/ --verbose  # With verbose output
```

### Your First HTMLx File

Create a file named `hello.htmlx`:
...
htmlx.exe input.htmlx output/
htmlx.exe input.htmlx output/ --verbose
htmlx.exe --version
htmlx.exe --help
...
For input file `mypage.htmlx`, the compiler generates:
...
```
htmlx/
├── HTMLx/                         # Main compiler package
│   ├── cli.py                     # Command-line interface
│   ├── parser/
│   │   ├── grammar.lark           # HTMLx grammar
...
├── example.htmlx                     # Full feature demonstration
├── language_server/               # Future LSP support (planned)
├── dist/
│   └── htmlx.exe                  # Standalone executable (Windows)
├── build/
│   └── htmlx/                     # Build directory with dependencies
...
pytest tests/unit/ --cov=HTMLx --cov-report=html
```
...
python cli.py input.htmlx ./dist/
```
...
4. **Real Examples**: Check `example.htmlx` for a complete demo
...
1. Give users `htmlx.exe` from the `dist/` folder
2. Users can compile ANY `.htmlx` file with one command:
   ```bash
   htmlx.exe mysite.htmlx output/
   ```
3. No Python installation, no dependencies, no configuration needed

### Deploy Generated Files

1. Compile your `.htmlx` file to get HTML, CSS, JS
...
5. **Share the compiler** - Give others `htmlx.exe` to build their own sites
...
2. Review the example.htmlx example
...
**HTMLx** - Because simpler markup means faster development. 🚀
