# Deployment Guide - HTMLx with Backend API

This guide explains how to deploy your HTMLx website and backend to the internet so it works for everyone.
...
The HTMLx compiler generates frontend files that need to know where to find your backend.
...
├── Web Server (Frontend HTML/CSS/JS)
└── Backend API Server (api.yourdomain.com or same server)
    ↓ (HTTPS)
    HTTP requests
```
...
The JavaScript generated by HTMLx is smart about finding your API:
...
3. **Environment Auto-Detection**
   - If NOT on localhost → Uses `/api` (relative URL)
   - If on localhost → Uses `http://localhost:5000/api`
...
// Auto-uses: http://localhost:5000/api ✓
```
...
// Auto-uses: /api (becomes https://yourdomain.com/api) ✓
```
...
// JavaScript uses: https://api.example.com ✓
```
...
```bash
# 1. Compile HTMLx
python cli.py comprehensive_test.htmlx output/
...
Open browser DevTools (F12) → Console tab to see JavaScript errors and API calls.
